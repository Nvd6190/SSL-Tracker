...
/** Convert a raw certificate date string to a full ISO datetime string. */
...
function scanCertificate(hostname) {
  return scanViaTLS(hostname).catch(() => scanViaHTTPS(hostname));
}

/**
 * Primary approach: raw TLS connect on port 443.
 */
function scanViaTLS(hostname) {
    ...
    const socket = tls.connect(
      ...
      () => {
        try {
          ...
          const validFrom = cert.valid_from ? toISODatetime(cert.valid_from) : null;
          const validTo = cert.valid_to ? toISODatetime(cert.valid_to) : null;
...
/**
 * Fallback: use an HTTPS request to retrieve the peer certificate.
 * Works for hosts behind CDNs or proxies that don't respond to raw TLS connect.
 */
function scanViaHTTPS(hostname) {
  return new Promise((resolve, reject) => {
    const req = https.request(
      ...
      (res) => {
        try {
          ...
          res.destroy();
          if (!cert || !cert.subject) {
            return reject(new Error('No certificate returned (HTTPS fallback)'));
          }
          ...
          const sanList = sanRaw
            ...
            .filter(Boolean);
          const hasWildcard = sanList.some((s) => s.startsWith('*.')) || cn.startsWith('*.');
          const certType = hasWildcard ? 'Wildcard' : 'Single Domain';
          const validFrom = cert.valid_from ? toISODatetime(cert.valid_from) : null;
          const validTo = cert.valid_to ? toISODatetime(cert.valid_to) : null;
    ...
    req.on('error', (err) => reject(new Error(`HTTPS fallback to ${hostname} failed — ${err.message}`)));
    req.on('timeout', () => { req.destroy(); reject(new Error(`HTTPS fallback to ${hostname} timed out`)); });
    req.end();
  });
}
